...
def save_selection(selection):
    ...
    Parameters:
    selection (dict): A dictionary containing the current selections of country, subarea, month, year, and accumulation_window.
    """
...
def month_year_interaction(btn_name, month_selector, year_selector, selected, placeholders):
    ...
    elif btn_name == 'year_range_widgets_btn' or btn_name == 'accumulation_windows_widgets_btn':
        year_selector.value = placeholders['year']
        selected['year'] = placeholders['year']
        month_selector.value = placeholders['month']
        selected['month'] = placeholders['month']        
...
def update_selected_values(selected, selectors, placeholders):
    ...
    selected.update({
        ...
        'accumulation_window': selectors['accumulation_window'].value if selectors['accumulation_window'].value != placeholders['accumulation_window'] else placeholders['accumulation_window'],
        ...
        'accumulation_windows_multiple': selectors['accumulation_windows_multiple'].value,
        'twenty_years': selectors['twenty_years'].value if selectors['twenty_years'].value != placeholders['twenty_years'] else placeholders['twenty_years'],
    })
...
def find_missing_selections(btn_name, selected, placeholders):
    ...
    if selected['accumulation_window'] == placeholders['accumulation_window'] and btn_name != 'accumulation_windows_widgets_btn':
        missing.append('accumulation_window')
    ...
    if btn_name == 'month_widgets_btn' and selected['month'] == placeholders['month']:
        ...
        missing.append('year')
    elif btn_name == 'year_range_widgets_btn' and not selected['year_range']:
        missing.append('year range')
    elif btn_name == 'accumulation_windows_widgets_btn':
        if len(selected['accumulation_windows_multiple']) < 2:
            missing.append('at least 2 accumulation windows')
        elif selected['twenty_years'] == placeholders['twenty_years']:
            missing.append('twenty years period')
    return missing
...
def get_period_of_time(btn_name, selected, placeholders):
    ...
    if btn_name == 'month_widgets_btn':
        ...
        time_period = f'year range {start_year}-{end_year}'
    elif btn_name == 'accumulation_windows_widgets_btn':
        start_year, end_year = map(int, selected['twenty_years'].split('-')) if selected['twenty_years'] != placeholders['twenty_years'] else 'undefined period'
        time_period = f'twenty years {start_year}-{end_year}'
    return time_period
